...
export default function ScenePopup({
  ...
  return (
    <div className={`w-full h-fit shrink-0 flex flex-col z-20 transition-all duration-300 ${isHorizontal ? 'bg-transparent border-none shadow-none p-0' : 'bg-white/95 backdrop-blur-md rounded-2xl shadow-[0_16px_40px_rgba(0,0,0,0.08)] border border-gray-100/80 overflow-hidden'}`}>
      ...
      <div className={isHorizontal ? "flex items-center gap-6 overflow-x-auto no-scrollbar py-1" : "p-5 overflow-y-auto flex-1 flex flex-col gap-6 scroll-smooth"} style={{ scrollBehavior: 'smooth' }}>
        ...
        {/* Sliders */}
        <div className={isHorizontal ? "flex items-center gap-4 shrink-0" : "flex flex-col gap-6 mt-1"}>
          <div className={`flex ${isHorizontal ? 'items-center gap-2 shrink-0' : 'flex-col gap-2.5'}`}>
            <label className="text-[10px] font-bold text-gray-600 flex justify-between gap-1 w-20 shrink-0">
            ...
            <input 
              ...
              className={`accent-[#c05520] ${isHorizontal ? 'w-20' : 'w-full'}`}
            />
          </div>
